...
contract ContinuosBondingERC20Token is BancorFormula, ERC20, ReentrancyGuard {
    ...
    function buyTokens() external payable {
        ...
        uint256 tokenBought = calculatePurchaseReturn(totalSupply(), totalETHContributed, uint32(RESERVE_RATIO), remainingAmount);
    ...
    function sellTokens(uint256 tokenAmount) external nonReentrant {
        ...
        uint256 reimburseAmount = calculateSaleReturn(totalSupply(), totalETHContributed, uint32(RESERVE_RATIO), tokenAmount);
    ...
    function calculateTokenAmount(uint256 ethAmount) public view returns (uint256) {
        calculatePurchaseReturn(totalSupply(), totalETHContributed, uint32(RESERVE_RATIO), ethAmount);
    }

    function calculateETHAmount(uint256 tokenAmount) public view returns (uint256) {
        calculateSaleReturn(totalSupply(), totalETHContributed, uint32(RESERVE_RATIO), tokenAmount);
    }
